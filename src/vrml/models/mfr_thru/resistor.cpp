...
int RESISTOR::makeHzLead( std::ofstream& fp )
{
    ...
    for( i = 0; i < nb; ++i )
        bend[i].SetNVertices( params.wsides );
...
int RESISTOR::makeBody( std::ofstream& fp, const std::string& bands )
{
    ...
    for( i = 0; i < ntot; ++i )
        body[i].SetNVertices( params.rsides );
    ...
    {
        tx.SetTranslation( params.shift * params.scale, 0, 2.0 * params.d * params.scale );
    }
...
int RESISTOR::Create( RPARAMS& aParameters, const std::string& aBandString,
                const std::string& aVRMLFname )
{
    ...
    for( i = 0; i < 14; ++i )
    {
        if( aParameters.colors[i].GetName().empty() )
        {
            ERRBLURB;
    ...
    // horiz: >= L + 4*d
    // vert:  >= D + 1.5*d
    if( aParameters.horiz )
    {
        if( aParameters.p < (aParameters.L + 4.0 * aParameters.d) )
        {
            ERRBLURB;
            cerr << "[horiz] lead pitch too small (" << setprecision( 4 ) <<
            "); must be >= L+4*d (" <<
                aParameters.L + 4.0 * aParameters.d << ")\n";
            fail = true;
        }
    }
    else
    {
        if( aParameters.p < (aParameters.D + 1.5 * aParameters.d) )
        {
            ERRBLURB;
            cerr << "[vert] lead pitch too small (" << setprecision( 4 ) <<
            "); must be >= D+1.5*d (" <<
                aParameters.D + 1.5 * aParameters.d << ")\n";
            fail = true;
        }
    }

    // ensure reasonable values for number of vertices in the circular sections
    if( (aParameters.wsides < 8)||(aParameters.wsides > 360) )
    {
        ERRBLURB;
        cerr << "invalid number of sides on wire (" << aParameters.wsides << "); valid range is 8..360\n";
        fail = true;
    }

    if( (aParameters.bsides < 4)||(aParameters.bsides > 24) )
    {
        ERRBLURB;
        cerr << "invalid number of sides on wire bend (" << aParameters.bsides <<
        "); valid range is 4..24\n";
        fail = true;
    }

    if( (aParameters.rsides < 3)||(aParameters.rsides > 360) )
    {
        ERRBLURB;
        cerr << "invalid number of sides on resistor (" << aParameters.rsides <<
        "); valid range is 3..360\n";
        fail = true;
    }

    if( (aParameters.endshape != 'C') && (aParameters.endshape != 'R') && (aParameters.endshape != 'B') )
    {
        ERRBLURB;
        cerr << "invalid end shape ('" << aParameters.endshape << "'); valid values are 'C', 'R', and 'B'\n";
        fail = true;
    }

    if( validateBands( aBandString ) )
    {
        ERRBLURB;
    ...
    params = aParameters;
    ostringstream fname;
    fname << aVRMLFname << params.endshape;

    if( !params.pwrsuf.empty() )
        fname << "_" << params.pwrsuf;

    if( !params.spcsuf.empty() )
        fname << "_" << params.spcsuf;
    ...
    {
        ERRBLURB;
        cerr << "could not open file: '" << aVRMLFname << "'\n";
        return -1;
    }
    ...
    {
        ERRBLURB;
        cerr << "problems writing to file '" << aVRMLFname << "'\n";
        return -1;
    }
