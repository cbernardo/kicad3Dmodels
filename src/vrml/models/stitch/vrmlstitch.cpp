...
int getRot( const std::string& val, ROTATION& rot )
{
    ...
    {
        ERRBLURB;
        cerr << "\tinvalid angle coefficient (" << setprecision( 12 ) << ang <<
        "); valid range is -2.0 .. 2.0\n";
        return -1;
    }
...
int process( std::ofstream& ofile, INFO& glob, INFO& part )
{
    ...
    while( ( file.good() ) && ( !file.eof() ) )
    {
        ...
        if( parsePart( ofile, glob, line, part, ofs ) )
            return -1;
    }
