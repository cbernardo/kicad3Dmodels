        ...
        VRMLMat &stemcolor, bool reuse_stem, std::ofstream &fp, int tabs)
{
    ...
    acc += poly[idx]->Stitch(*poly[idx +1], true, t, flutecolor, reuse, fp, tabs);
    ++idx;
    if (has_h2)
    {
        acc += poly[idx]->Stitch(*poly[idx +1], true, t, flutecolor, true, fp, tabs);
        ++idx;
    }
    // stem
    reuse = reuse_stem;
    ...
    acc += poly[idx]->Stitch(*poly[idx +1], true, t, stemcolor, reuse, fp, tabs);
    ++idx;
    if (cap)
    {
