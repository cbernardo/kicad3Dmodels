...
int SetupVRML(std::string filename, std::ofstream &fp)
{
    ...
    fp << "#    the end user is free to use, modify, and distribute this\n";
    fp << "#    file without restrictions.\n";
    fp << "\n";
    if (!fp.good())
    {
        ERRBLURB;
...
int SetupGeom(std::ofstream &fp, int tabs)
{
    if (!fp.good())
    {
        ERRBLURB;
        cerr << "bad stream for VRML Geometry\n";
        return -1;
    }
